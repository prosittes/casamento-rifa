...
const modalPresenteValor = document.getElementById('modal-presente-valor');
...
let presenteSelecionadoValor = null;
      ...
      html += `
        <div class="presente-card ${reservado ? 'reservado' : ''}">
          ...
          ${reservado
            ...
            : `<button class="btn-escolher" data-id="${p.id}" data-nome="${p.nome}" data-valor="${p.valor || ''}" data-img="${p.imagem || ''}">Escolher</button>`
          }
        </div>
      `;
    });
...
function abrirModal(id, nome, valor, img) {
  ...
  presenteSelecionadoValor = valor;
  ...
  modalPresenteValor.textContent = valor ? `Valor sugerido: ${valor}` : '';
  modalPresenteValor.style.display = valor ? 'block' : 'none';
  inputPadrinho.value = '';
